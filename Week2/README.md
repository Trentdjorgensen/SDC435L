...
This Python application uses Redis to store, retrieve, update, delete, and analyze data from the GitHub Archive dataset. The program reads JSON-formatted GitHub data and stores selected information in MongoDB so users can perform CRUD operations and view additional analysis features.

## Features

- Connects to a local MongoDB using Python.
- Reads JSON-formatted data from the GitHub Archive dataset.
- Automatically loads repository, commit, and programming language data into MongoDB.
...
- Includes error handling for missing files, invalid input, and MongoDB connection problems.
